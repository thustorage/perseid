...
#include "db/sec_idx/sec_idx_common.h"
#include "port/port.h"
...
struct Saver {
  Saver() : state(), ucmp(), user_key(), value(), pseq() {}
  SaverState state;
...
class Version {
 public:
  ...
  Status Get(const ReadOptions&, const LookupKey& key, std::string* val,
             GetStats* stats, SequenceNumber* pseq = nullptr);
  Status SGetPKeyOnly(const ReadOptions& options, const LookupKey& key,
                      std::vector<std::string>* pkey_list, int kNoOfOutputs,
                      std::set<std::string>* result_keys_found, DB* pri_db,
                      DB* pri_key_index);
  Status SGet(const ReadOptions& options, const LookupKey& k,
...
class VersionSet {
  ...
  */
  ...
  int NumMaxGuardFiles(unsigned level) const;
...
class Compaction {
 ...
 private:
  ...
  explicit Compaction(const Options* options, unsigned level);
