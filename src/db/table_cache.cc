...
Status TableCache::FindTable(uint64_t file_number, uint64_t file_size,
	...
	*handle = cache_->Lookup(key);
	if (*handle == NULL) {
		...
		s = env_->NewRandomAccessFile(fname, &file, options_->use_mmap_read);
		record_timer(GET_TABLE_CACHE_GET_NEW_RANDOM_ACCESS_FILE);
		if (!s.ok()) {
			std::string old_fname = LDBTableFileName(dbname_, file_number);
			if (env_->NewRandomAccessFile(old_fname, &file, options_->use_mmap_read).ok()) {
				s = Status::OK();
			}
		}
		if (s.ok()) {
...
Status TableCache::Get(const ReadOptions& options,
  ...
  if (s.ok()) {
    ...
    if (this->options_->using_s_index) {
      StopWatch<uint64_t> sw((uint64_t&)options.time_pri_file_io);
      // bool skip_filter_check =
      //     config::kPruneComponentsBySeq && options.sec_read;
