...
static double MaxBytesPerGuardForLevel(const Options* options, unsigned level) {
  assert(level < leveldb::config::kNumLevels);
  static const double bytes[] = {64 * 1048576.0,
                                 128 * 1048576.0,
                                 256 * 1048576.0,
...
static uint64_t MinFileSizeForLevel(const Options* options, unsigned level) {
  assert(level < leveldb::config::kNumLevels);
...
static uint64_t MaxFileSizeForLevel(const Options* options, unsigned level) {
  assert(level < leveldb::config::kNumLevels);
...
Status Version::Get(const ReadOptions& options,
  ...
  for (unsigned level = 0; level < config::kNumLevels; level++) {
    ...
    // less
    // than the guard key of the first guard, it means that the key might be
    // present in one of the sentinel files of that level.

    vstart_timer(GET_FIND_LIST_OF_FILES, BEGIN, 1);
    if (num_guards == 0  // If there are no guards in the level, look at the
                         // sentinel files
        || (guard_index ==
                0  // If there are guards in the level and guard_index is 0, key
                   // can either be in sentinel or in the first(0-index) guard
            && num_guards > 0 &&
      ...
      for (size_t i = 0; i < sentinel_files_[level].size(); i++) {
        ...
        // Optimization: Adding only the files where the required key lies
        // between smallest and largest
        if (ucmp->Compare(user_key, f->smallest.user_key()) >= 0 &&
            ucmp->Compare(user_key, f->largest.user_key()) <= 0) {
          if (!config::kPruneComponentsBySeq || !pseq ||
              (f->min_seq <= *pseq && f->max_seq >= *pseq)) {
            tmp2.push_back(f);
          }
        }
      }
      ...
      // TODO: If this sorting is moved to LogAndApply, sorting need not be done
      // during read
    ...
    for (uint32_t i = 0; i < num_files; ++i) {
      ...
      num_files_read++;
      if (this->vset_->options_->using_s_index) {
      ...
      if (s.ok() && !s.IsNotFound() && saver.state == kFound) {
        ...
        while (p < end && kNoOfOutputs > 0) {
          ...
          if (!resultSetofKeysFound->contains(pkey)) {
            ...
            if (!pass) {
              share->pkey_vec[share->head] = PKeyItem(cur_seq, pkey);
              ++total_cnt;
              share->head.store(total_cnt, std::memory_order_release);
              --kNoOfOutputs;
...
Status Version::SGetPKeyOnly(const ReadOptions& options, const LookupKey& key,
                             std::vector<std::string>* pkey_list,
                             int kNoOfOutputs,
                             std::set<std::string>* result_keys_found,
                             DB* pri_db, DB* pri_key_index) {
...
#else
    return Status::NotFound("not supported");
#endif
...
Status Version::SGet(const ReadOptions& options, const LookupKey& k,
...
std::string Version::DebugString() const {
  ...
  for (unsigned level = 0; level < config::kNumLevels; level++) {
    ...
    for (size_t i = 0; i < files.size(); i++) {
      ...
      r.append("{");
      AppendNumberTo(&r, files[i]->min_seq);
      r.append(" .. ");
      AppendNumberTo(&r, files[i]->max_seq);
      r.append("}");
      r.append("[");
...
class VersionSet::Builder {
 private:
  ...
  struct LevelState {
    LevelState() : deleted_files(), added_guards(), added_complete_guards(), deleted_guards(), added_files() {}
...
void VersionSet::Finalize(Version* v) {
  ...
  for (unsigned level = 0; level < config::kNumLevels; ++level) {
    ...
    if (level == 0) {
      ...
      score1 = sentinel_bytes / MaxBytesPerGuardForLevel(options_, level);
      ...
      for (unsigned i = 0; i < num_guards; i++) {
    	  ...
    	  score1 = guard_file_bytes / MaxBytesPerGuardForLevel(options_, level);
...
Status VersionSet::WriteSnapshot(log::Writer* log) {
  ...
  for (unsigned level = 0; level < config::kNumLevels; level++) {
    ...
    for (size_t i = 0; i < files.size(); i++) {
      const FileMetaData* f = files[i];
      edit.AddFile(level, f->number, f->file_size, f->smallest, f->largest, f->min_seq, f->max_seq);
    }
...
int VersionSet::NumMaxGuardFiles(unsigned level) const {
  assert(level < config::kNumLevels);
  int f = current_->sentinel_files_[level].size();
  for (unsigned i = 0; i < current_->guards_[level].size(); i++) {
    f = std::max(f, (int)current_->guards_[level][i]->files.size());
  }
  return f;
}
...
Compaction::Compaction(const Options* options, unsigned l)
    : level_(l),
      min_output_file_size_(MinFileSizeForLevel(options, l)),
      max_output_file_size_(MaxFileSizeForLevel(options, l)),
      input_version_(NULL),
      edit_(),
      boundaries_() {
