...
class masstree {
    private:
        ...
        PMemAllocator *pmem_allocator_ = nullptr;
    ...
    public:
        ...
        MASS::ThreadInfo *getpThreadInfo();
        ...
        void put(const Slice &key, const Slice &value, SequenceNumber seq,
                 MASS::ThreadInfo &threadEpocheInfo, clht *ht);
        ...
        void *get(const Slice &key, MASS::ThreadInfo &threadEpocheInfo);
        ...
        leafvalue *make_leaf(const char *key, size_t key_len, uint64_t value);
        ...
        int scan(const Slice &skey, int num, uint64_t *buf,
...
class leafnode {
    ...
    public:
        ...
        int split_into_pkey(masstree *t, leafnode *nr, int p,
        ...
        void *leaf_insert_pkey(masstree *t, void *root, uint32_t depth,
                               uint64_t fkey, const Slice &key,
                               const Slice &value, SequenceNumber seq,
                               key_indexed_position &kx_, clht *ht);

        void leaf_update_pkey(masstree *t, key_indexed_position &kx_,
        ...
        void make_new_layer(masstree *t, leafnode *p, key_indexed_position &kx_, leafvalue *olv, leafvalue *nlv, uint32_t depth);
        ...
        void get_slot_range_in_same_pkey_block(int cur_slot, int *min_slot,
                                               int *max_slot);

        char *insert_and_split_to_new_pkey_block(masstree *t, int ins_slot,
                                                 int min_slot, int max_slot,
                                                 bool update, PKeyBlock *cur_pb,
                                                 bool &free_cur_pb,
                                                 const Slice &value,
                                                 SequenceNumber seq, clht *ht);

        void split_to_new_pkey_block(masstree *t, int min_slot, int max_slot,
                                     clht *ht);
};
