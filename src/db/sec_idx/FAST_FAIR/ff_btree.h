...
#include "db/sec_idx/pkey_block.h"
...
class btree {
 ...
 public:
  ...
  PMemAllocator *pkey_block_allocator_ = nullptr;
...
class alignas(PAGESIZE) page {
 ...
 public:
  ...
  inline void insert_pkey(btree *bt, entry_key_t key, const Slice &value,
                          SequenceNumber seq, clht *ht, int ins_slot,
                          int *num_entries, bool flush = true,
                          bool update_last_index = true) {
    assert(*num_entries == count());
    ...
    if (*num_entries == 0) {  // this page is empty
      PKeyBlock *pb =
          (PKeyBlock *)bt->pkey_block_allocator_->Alloc(PKEY_BLOCK_SIZE);
      pb->init();
      ...
      assert(*num_entries < cardinality - 1);
      records[*num_entries + 1].ptr = records[*num_entries].ptr;
      if (flush) {
        if ((uint64_t) & (records[*num_entries + 1].ptr) % CACHE_LINE_SIZE == 0)
          idx_clwb_fence((char *)&(records[*num_entries + 1].ptr),
      ...
      int prev = ins_slot > 0 ? ins_slot - 1 : ins_slot;
      PKeyBlock *pb = get_pkey_block_addr(records[prev].ptr);
      if (!pb) {
        pb = (PKeyBlock *)bt->pkey_block_allocator_->Alloc(PKEY_BLOCK_SIZE);
        pb->init();
      }
      char *ptr = pb->Append(value, seq, nullptr);
      if (ptr) {
        for (int i = *num_entries - 1; i >= ins_slot; i--) {
          records[i + 1].ptr = records[i].ptr;
          records[i + 1].key = records[i].key;
        ...
        if (ins_slot > 0) {
          records[ins_slot].ptr = records[ins_slot + 1].ptr;
          records[ins_slot].key = key;
          records[ins_slot].ptr = ptr;
          if (flush) idx_clwb_fence((char *)&records[ins_slot], sizeof(entry));
        } else {
          records[0].ptr = (char *)hdr.leftmost_ptr;
          records[0].key = key;
          records[0].ptr = ptr;
          if (flush) idx_clwb_fence((char *)&records[0], sizeof(entry));
        }
      } else {
        // need split
        std::vector<char *> new_ptr;
        int min_slot, max_slot;
        get_slot_range_in_same_pkey_block(prev, *num_entries, &min_slot,
                                          &max_slot);
        bool free_cur_pb = false;
        insert_and_split_to_new_pkey_block(bt, ins_slot, min_slot, max_slot,
                                           false, pb, free_cur_pb, value, seq,
                                           ht, new_ptr);
        // note that new_ptr contains [min_slot, max_slot + 1]
        for (int i = *num_entries - 1; i > max_slot; i--) {
          records[i + 1].ptr = records[i].ptr;
          records[i + 1].key = records[i].key;
        ...
        // shift one by insert_slot
        for (int i = max_slot + 1; i > ins_slot; i--) {
          assert(i < cardinality);
          records[i].ptr = new_ptr[i - min_slot];
          records[i].key = records[i - 1].key;
          if (flush) {
            uint64_t records_ptr = (uint64_t)(&records[i + 1]);
            int remainder = records_ptr % CACHE_LINE_SIZE;
        ...
        if (ins_slot > 0) {
          records[ins_slot].ptr = records[ins_slot - 1].ptr;
          records[ins_slot].key = key;
          records[ins_slot].ptr = new_ptr[ins_slot - min_slot];
          if (flush) idx_clwb_fence((char *)&records[ins_slot], sizeof(entry));
        } else {
          records[0].ptr = (char *)hdr.leftmost_ptr;
          records[0].key = key;
          records[0].ptr = new_ptr[ins_slot - min_slot];
          if (flush) idx_clwb_fence((char *)&records[0], sizeof(entry));
        }
        for (int i = ins_slot - 1; i >= min_slot; i--) {
          records[i].ptr = new_ptr[i - min_slot];
        }
        if (flush && ins_slot - 1 >= min_slot) {
          idx_clwb_fence(&records[min_slot],
                         sizeof(entry) * (ins_slot - min_slot));
        }
        if (free_cur_pb) {
          bt->pkey_block_allocator_->Free(pb, PKEY_BLOCK_SIZE);
        }
      }
  ...
  page *store(btree *bt, entry_key_t key, const Slice &value,
              SequenceNumber seq, clht *ht, bool flush, bool with_lock,
    ...
    if (hdr.sibling_ptr && (hdr.sibling_ptr != invalid_sibling)) {
      ...
      // if (key >= hdr.sibling_ptr->records[0].key) {
        ...
        return hdr.sibling_ptr->store(bt, key, value, seq, ht, flush, with_lock,
                                      invalid_sibling);
      }
    }
    ...
    for (i = 0; i < num_entries; i++) {
      if (records[i].key == key) {
        PKeyBlock *pb = get_pkey_block_addr(records[i].ptr);
        char *ret = pb->Append(value, seq, records[i].ptr);
        if (ret == nullptr) {
          std::vector<char *> new_ptr;
          int min_slot, max_slot;
          get_slot_range_in_same_pkey_block(i, num_entries, &min_slot,
                                            &max_slot);
          bool free_cur_pb = false;
          insert_and_split_to_new_pkey_block(bt, i, min_slot, max_slot, true,
                                             pb, free_cur_pb, value, seq, ht,
                                             new_ptr);
          for (int k = min_slot; k <= max_slot; k++) {
            records[k].ptr = new_ptr[k - min_slot];
          }
          if (flush) {
            idx_clwb_fence(&records[min_slot],
                           sizeof(entry) * (max_slot - min_slot + 1));
          }
          if (free_cur_pb) {
            // free old pkey block
            bt->pkey_block_allocator_->Free(pb, PKEY_BLOCK_SIZE);
    ...
    // FAST
    if (num_entries < cardinality - 1) {
      insert_pkey(bt, key, value, seq, ht, insert_slot, &num_entries, flush,
                  true);
      if (with_lock) {
        hdr.mtx->unlock();  // Unlock the write lock
      }
      ...
      int m = (int)ceil(num_entries / 2);
      entry_key_t split_key = records[m].key;
      PKeyBlock *pb_left = get_pkey_block_addr(records[m - 1].ptr);
      PKeyBlock *pb_right = get_pkey_block_addr(records[m].ptr);
      if (pb_left == pb_right) {
        // split pkey block first
        int m_min_slot, m_max_slot;
        get_slot_range_in_same_pkey_block(m, num_entries, &m_min_slot,
                                          &m_max_slot);
        std::vector<char *> new_ptr;
        // new right part
        split_to_new_pkey_block(bt, m, m_max_slot, ht, new_ptr);

        for (int i = m; i <= m_max_slot; i++) {
          sibling->records[i - m].key = records[i].key;
          sibling->records[i - m].ptr = new_ptr[i - m];
        }
        for (int i = m_max_slot + 1; i < num_entries; i++) {
          sibling->records[i - m].key = records[i].key;
          sibling->records[i - m].ptr = records[i].ptr;
        }
        sibling_cnt = num_entries - m;
        sibling->hdr.last_index = sibling_cnt - 1;
        sibling->hdr.fence_key = records[m].key;
        sibling->hdr.sibling_ptr = hdr.sibling_ptr;
        idx_clwb_fence(sibling, sizeof(page));

        // left part
        split_to_new_pkey_block(bt, m_min_slot, m - 1, ht, new_ptr);
        for (int i = m_min_slot; i <= m - 1; i++) {
          records[i].ptr = new_ptr[i - m_min_slot];
        }
        idx_clwb_fence(&records[m_min_slot], sizeof(entry) * (m - m_min_slot));
        hdr.sibling_ptr = sibling;
        idx_clwb_fence(&hdr, sizeof(hdr));
        ...
        bt->pkey_block_allocator_->Free(pb_left, PKEY_BLOCK_SIZE);
      } else {
        // luckily, they are already splitted
        // migrate half of keys into the sibling
        assert(hdr.leftmost_ptr == NULL);  // leaf node
        for (int i = m; i < num_entries; i++) {
          sibling->records[i - m].key = records[i].key;
          sibling->records[i - m].ptr = records[i].ptr;
        }
      ...
      page *ret;
      if (insert_slot <= m) {
        insert_pkey(bt, key, value, seq, ht, insert_slot, &num_entries, true,
                    true);
        ret = this;
      } else {
        sibling->insert_pkey(bt, key, value, seq, ht, insert_slot - m,
                             &sibling_cnt, true, true);
        ret = sibling;
      }
  ...
  void get_slot_range_in_same_pkey_block(int cur_slot, int num_entries,
                                         int *min_slot, int *max_slot) {
    assert(min_slot && max_slot);
    PKeyBlock *cur_pb = get_pkey_block_addr(records[cur_slot].ptr);
    int i;
    for (i = cur_slot - 1; i >= 0; i--) {
      PKeyBlock *p = get_pkey_block_addr(records[i].ptr);
  ...
  /**
   * @param ins_slot: the slot where the key should be updated or inserted
   * @return
   */
  char *insert_and_split_to_new_pkey_block(
      btree *bt, int ins_slot, int min_slot, int max_slot, bool update,
      PKeyBlock *cur_pb, bool &free_cur_pb, const Slice &value,
      SequenceNumber seq, clht *ht, std::vector<char *> &new_ptr) {
    new_ptr.clear();
    assert(min_slot <= max_slot);
    int vec_sz = update ? max_slot - min_slot + 1 : max_slot - min_slot + 2;
    new_ptr.resize(vec_sz, nullptr);
    assert(ins_slot == max_slot + 1 ||
           get_pkey_block_addr(records[ins_slot].ptr) == cur_pb);
    // note ins_slot may equals max_slot + 1
    ...
    for (int j = 0; j < vec_sz; j++) {
      ...
      if (j != ins_slot - min_slot) {
        // other slots
        PKeyHeader *ph = (PKeyHeader *)records[i].ptr;
        ...
        if (cur_cnt > 0) {
          ...
          new_ptr[j] = new_pb->AppendBatchNoPersist(buffer, cur_cnt, ph);
        } else {
          if (ph && (PKeyBlock *)ph != get_pkey_block_addr(ph) &&
            ...
            char *ptr = new_pb->CollectPKeysAndAppendMultiBlocks(
                bt->pkey_block_allocator_, ph, ht);
            if (ptr) {
              new_pb = get_pkey_block_addr(ptr);
              new_ptr[j] = ptr;
            } else {
              new_ptr[j] = (char *)new_pb;
            }
          } else {
            new_ptr[j] = (char *)new_pb;
          }
        ...
        if (update) {
          // append old pkeys
          ph = (PKeyHeader *)records[ins_slot].ptr;
          // for each discrete pkeys
          while (ph && (PKeyBlock *)ph != cur_pb &&
                 get_pkey_block_addr(ph) == cur_pb) {
            free_cur_pb = true;
        ...
        char *cur_ptr = new_pb->AppendMultiBlocks(bt->pkey_block_allocator_,
                                                  buffer, offset, ph);
        assert(cur_ptr);
        ...
        for (PKeyBlock *old_pb : gc_set) {
          bt->pkey_block_allocator_->Free(old_pb, PKEY_BLOCK_SIZE);
        }
      }
    }
    new_pb->Persist();
  ...
  void split_to_new_pkey_block(btree *bt, int min_slot, int max_slot, clht *ht,
                               std::vector<char *> &new_ptr) {
    new_ptr.resize(max_slot - min_slot + 1, nullptr);
    PKeyBlock *cur_pb = get_pkey_block_addr(records[min_slot].ptr);
    PKeyBlock *new_pb =
        (PKeyBlock *)bt->pkey_block_allocator_->Alloc(PKEY_BLOCK_SIZE);
    new_pb->init();

    for (int i = min_slot, j = 0; i <= max_slot; i++, j++) {
      std::string buffer;
      buffer.reserve(512);
      int cur_cnt = 0;
      PKeyHeader *ph = (PKeyHeader *)records[i].ptr;
