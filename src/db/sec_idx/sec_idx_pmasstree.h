...
class SecIdxPMasstree : public SecIdx {
 public:
  SecIdxPMasstree() {
  ...
  virtual Status SInsert(const Slice &skey, const Slice &pkey,
                         SequenceNumber seq) {
    mt_->put(skey, pkey, seq, *mt_ti_, vol_pri_key_idx_);
    if (leveldb::config::kUsingValidation) {
  ...
  virtual Status SGet(const ReadOptions &options, const Slice &skey,
    ...
    PKeyHeader *ph = (PKeyHeader *)mt_->get(skey, *mt_ti_);
          ...
          if (!result_keys_found.contains(pkey)) {
            ...
            if (!check_skip) {
              SGetLocalNaive &cur_local =
                  sget_local_naive_[cur_parallel_thread];
              cur_local.pkey_vec[cur_local.head] = PKeyItem(pke_seq, pkey);
              cur_local.head.fetch_add(1, std::memory_order_release);
              --num_rec;
              result_keys_found.insert(pkey);
              ++total_cnt;
              cur_parallel_thread =
                  (cur_parallel_thread + 1) % num_parallel_threads;
            }
          }
        }
        pke = (PKeyEntry *)((char *)pke + sizeof(PKeyEntry) + sz);
