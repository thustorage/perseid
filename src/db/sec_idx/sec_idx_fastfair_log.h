...
#include "CCEH/CCEH.h"
#include "FAST_FAIR-log/ff_btree.h"
...
class SecIdxFastFairLog : public SecIdx {
 public:
  SecIdxFastFairLog() {
    btree_ = new btree();
  ...
  virtual Status SInsert(const Slice &skey, const Slice &pkey,
                         SequenceNumber seq) {
    btree_->btree_insert(skey, pkey, seq);
    if (leveldb::config::kUsingValidation) {
  ...
  virtual Status SGet(const ReadOptions &options, const Slice &skey,
    ...
    PKeyEntry *pe = (PKeyEntry *)btree_->btree_search(skey);
