...
masstree::masstree() {
  pmem_allocator_ = new MMAPAllocator("masstree_pool", 20ul << 30);
...
leafvalue *leafnode::max_leaf(size_t key_len, uint64_t value) {
  void *aligned_alloc;
  size_t len = (key_len % sizeof(uint64_t)) == 0
                   ? key_len
                   : (((key_len) / sizeof(uint64_t)) + 1) * sizeof(uint64_t);
...
void masstree::put(const Slice &key, uint64_t value,
...
leaf_retry:
  ...
  if (kx_.p >= 0) {
    ...
    if (!IS_LV(l->value(kx_.p))) {
      ...
      // new layer
      leafvalue *lv = make_leaf(key.data(), key.size(), value);
...
void leafnode::get_range(leafvalue *&min_lv, leafvalue *&max_lv, int num,
                         int &count, std::vector<std::string> *pkey_list,
                         std::vector<uint64_t> *seq_list, leafnode *root,
                         uint32_t depth, uint64_t &validate_time, clht *ht) {
  key_indexed_position kx_;
  leafnode *next = NULL;
  void *snapshot_v = NULL, *snapshot_n = NULL;
  permuter perm;
...
restart:
  ...
  while (p->level() != 0) {
  inter_retry:
    next = p->advance_to_key(min_lv->fkey[depth]);
    if (next != p) {
      p = next;
      goto inter_retry;
    }
    ...
    kx_ = p->key_lower_bound(min_lv->fkey[depth]);
  ...
  leaf_retry:
    ...
    for (int i = 0; i < perm.size() && count < num; i++) {
      ...
      if (!IS_LV(l->value(perm[i]))) {
        p = reinterpret_cast<leafnode *>(snapshot_v);
        if (l->key(perm[i]) > max_lv->fkey[depth]) {
          stop = true;
          break;
        } else if (l->key(perm[i]) == max_lv->fkey[depth]) {
          leafvalue *largest = p->max_leaf(max_lv->key_len, max_lv->value);
          if (l->key(perm[i]) > min_lv->fkey[depth]) {
            leafvalue *smallest =
                p->smallest_leaf(min_lv->key_len, min_lv->value);
            p->get_range(smallest, largest, num, count, pkey_list, seq_list, p,
                         depth + 1, validate_time, ht);
            free(smallest);
          } else if (l->key(perm[i]) == min_lv->fkey[depth]) {
            p->get_range(min_lv, largest, num, count, pkey_list, seq_list, p,
                         depth + 1, validate_time, ht);
          }
          free(largest);
        } else {
          ...
          if (l->key(perm[i]) > min_lv->fkey[depth]) {
            leafvalue *smallest =
                p->smallest_leaf(min_lv->key_len, min_lv->value);
            p->get_range(smallest, max_lv, num, count, pkey_list, seq_list, p,
                         depth + 1, validate_time, ht);
            free(smallest);
          } else if (l->key(perm[i]) == min_lv->fkey[depth]) {
            p->get_range(min_lv, max_lv, num, count, pkey_list, seq_list, p,
                         depth + 1, validate_time, ht);
          }
        }
      } else {
        snapshot_v = (LV_PTR(snapshot_v));
        if (l->key(perm[i]) >= max_lv->fkey[depth] ||
            keycmp((LV_PTR(l->value(perm[i])))->fkey, max_lv->fkey,
                   max_lv->key_len) >= 0) {
          stop = true;
          break;
        } else if (l->key(perm[i]) > min_lv->fkey[depth] ||
                   keycmp((LV_PTR(l->value(perm[i])))->fkey, min_lv->fkey,
                          min_lv->key_len) >= 0) {
          leafvalue *cur_lv = reinterpret_cast<leafvalue *>(snapshot_v);
          SeqVal *sv = (SeqVal *)&cur_lv->value;
          if (!sv->IsDeleted()) {
            // TODO: for now, only 8-byte key
            // StopWatch<uint64_t> t((uint64_t &)validate_time);
            uint64_t pk = __builtin_bswap64(cur_lv->fkey[1]);
            uint64_t pke_seq = sv->seq_;
            std::string pkey((char *)&pk, sizeof(uint64_t));
            uint64_t pkey_hash = pkey.size() <= sizeof(uint64_t)
                                     ? *(uint64_t *)pkey.data()
                                     : MurmurHash64A(pkey.data(), pkey.size());
            bool eq = clht_check_seqcnt(ht->ht, pkey_hash, pke_seq);
            if (eq) {
              pkey_list->emplace_back(
                  std::string((const char *)&pk, sizeof(uint64_t)));
              seq_list->push_back(pke_seq);
              ++count;
            } else {
              // TODO: delete
              sv->SetDeleted();
...
int masstree::scan(const Slice &min, const Slice &max, int num,
                   std::vector<std::string> *pkey_list,
                   std::vector<uint64_t> *seq_list,
                   MASS::ThreadInfo &threadEpocheInfo, uint64_t &validate_time,
                   clht *ht) {
  EpocheGuard epocheGuard(threadEpocheInfo);
  void *root = NULL;
  key_indexed_position kx_;
  ...
  leafvalue *min_lv = make_leaf(min.data(), min.size(), 0);
  leafvalue *max_lv = make_leaf(max.data(), max.size(), 0);
  int needRestart;
  uint64_t v;
