...
#include "CCEH/CCEH.h"
#include "FAST_FAIR-composite/ff_btree.h"
...
class SecIdxFastFairComposite : public SecIdx {
 public:
  SecIdxFastFairComposite() {
    btree_ = new btree();
  ...
  virtual ~SecIdxFastFairComposite() {
  ...
  virtual Status SGet(const ReadOptions &options, const Slice &skey,
    ...
    btree_->btree_search_range(sk, sk_max, num_rec, &pkey_list, &seq_list,
...
// parallel sget
  ...
  virtual Status SScanPKeyOnly(const ReadOptions &options, const Slice &skey,
    ...
    for (int i = 0; i < num_rec; ++i) {
      ...
      std::vector<uint64_t> seq_list;
      btree_->btree_search_range(
          cur_sk, sk_max, options.num_scan_pkey_cnt, &pkeys, &seq_list,
          (uint64_t &)options.time_validate, vol_pri_key_idx_);
      if (pkeys.size() > 0) {
