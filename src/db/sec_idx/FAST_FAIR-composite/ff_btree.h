...
struct SeqVal {
  ...
  // SeqVal() {}
  bool IsDeleted() { return delete_ == true; }
  void SetDeleted() { delete_ = true; }
};
...
class btree {
 ...
 public:
  ...
  void btree_insert(entry_key_t key, uint64_t value);
  ...
  void btree_search_range(entry_key_t min, entry_key_t max, int cnt,
                          std::vector<std::string> *pkey_list,
                          std::vector<uint64_t> *seq_list,
                          uint64_t &validate_time, clht *ht);
  // void printAll();
...
class alignas(PAGESIZE) page {
 ...
 public:
  ...
  page *store(btree *bt, entry_key_t key, char *right, bool flush,
    ...
    if (hdr.sibling_ptr && (hdr.sibling_ptr != invalid_sibling)) {
      ...
      if (key >= hdr.sibling_ptr->hdr.fence_key) {
        ...
        return hdr.sibling_ptr->store(bt, key, right, true, with_lock,
                                      invalid_sibling);
      }
    }
    ...
    if (num_entries < cardinality - 1) {
      ...
      if (key < split_key) {
        ...
        sibling->insert_key(key, right, &sibling_cnt, flush, true);
        ret = sibling;
      }
  ...
  void linear_search_range(entry_key_t min, entry_key_t max, int cnt,
                           std::vector<std::string> *pkey_list,
                           std::vector<uint64_t> *seq_list,
                           uint64_t &validate_time, clht *ht) {
    int i, off = 0;
      ...
      do {
        ...
        if (IS_FORWARD(previous_switch_counter)) {
          if ((tmp_key = current->records[0].key) > min) {
            if (pkey_list->size() < cnt && tmp_key < max) {
              if ((tmp_ptr = current->records[0].ptr) != NULL) {
                if (tmp_key == current->records[0].key) {
                  if (tmp_ptr) {
                    SeqVal *sv = (SeqVal *)&current->records[0].ptr;
                    if (!sv->IsDeleted()) {
                      std::string pkey(tmp_key.key + 8, sizeof(uint64_t));
                      ...
                      if (eq) {
                        seq_list->push_back((unsigned long)sv->seq_);
                        pkey_list->emplace_back(tmp_key.key + 8,
                                                sizeof(uint64_t));
                      } else {
                        sv->SetDeleted();
                      }
              ...
              if (pkey_list->size() < cnt && tmp_key < max) {
                if ((tmp_ptr = current->records[i].ptr) !=
                  ...
                  if (tmp_key == current->records[i].key) {
                    if (tmp_ptr) {
                      SeqVal *sv = (SeqVal *)&current->records[i].ptr;
                      if (!sv->IsDeleted()) {
                        std::string pkey(tmp_key.key + 8, sizeof(uint64_t));
                        uint64_t pke_seq = sv->seq_;
                        uint64_t pkey_hash =
