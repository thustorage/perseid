...
btree::btree() {
  pmem_allocator_ = new MMAPAllocator("fastfair_pool", 20ul << 30);
...
void btree::btree_insert(entry_key_t key, uint64_t val) {
  page *p = (page *)root;
  ...
  if (!p->store(this, key, (char *)val, true, true, nullptr)) {
    btree_insert(key, val);
  }
}
...
void btree::btree_insert_internal(char *left, entry_key_t key, char *right,
  ...
  if (!p->store(this, key, right, true, true)) {
...
void btree::btree_search_range(entry_key_t min, entry_key_t max, int cnt,
  ...
  while (p) {
    if (p->hdr.leftmost_ptr != NULL) {
      ...
      // Found a leaf
      p->linear_search_range(min, max, cnt, pkey_list, seq_list, validate_time,
                             ht);
      break;
    }
  }
