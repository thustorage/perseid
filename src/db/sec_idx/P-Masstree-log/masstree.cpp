...
leaf_retry:
  ...
  if (kx_.p >= 0) {
    ...
    if (!IS_LV(l->value(kx_.p))) {
      ...
      uint64_t old_prev = (LV_PTR(l->value(kx_.p)))->value;
      char *new_val = pkey_log_->Append(value, seq, (void *)old_prev);
      (LV_PTR(l->value(kx_.p)))->value = (uint64_t)new_val;
      clflush((char *)&(LV_PTR(l->value(kx_.p)))->value, sizeof(void *), false,
              true);
      l->writeUnlock(false);
      // iii) Allocate additional layers (B+tree's roots) up to
      ...
      // new layer
      char *new_val = pkey_log_->Append(value, seq, nullptr);
    ...
    if (!(l->leaf_insert(this, root, depth, lv, lv->fkey[depth], SET_LV(lv),
                         kx_))) {
      put(key, value, seq, threadEpocheInfo);
    }
  }
  free(tmp_lv);
}
